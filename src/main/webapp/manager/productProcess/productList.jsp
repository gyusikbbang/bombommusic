...
<body>
	<div class="container-fluid">
		...
		<form name="search"
			action="/BomBom/manager/productProcess/productSearch.jsp"
			method="post">
			<select name="ProCate">
...
track_<%=i%>.addEventListener("ended", function() {
  isPlaying_<%=i%> = false;
					...
					<td>
						...
						<button onclick="insertMd_pro(<%=product.getProCode()%>)" class="btn8">mdPick에 추가</button>
...
<script>
function insertMd_pro(proCode) {
    if (confirm("추가하시려면 확인  원치 않으시면 취소 선택해주세요.")) {
        location.href='/BomBom/manager/productProcess/productMdpro.jsp?proCode=' + proCode;
    } else {
        alert('원래 화면으로 돌아갑니다');
        location.href = "/BomBom/manager/productProcess/productList.jsp?proCate=all";
    }
