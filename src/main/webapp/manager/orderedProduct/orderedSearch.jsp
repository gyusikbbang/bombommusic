...
count = buyprocess.searchcount(memberId);

if (count > 0) {
	buyList = buyprocess.search(memberId, startRow, endRow);
...
<body>
	<div class="container-fluid">
		...
		<div>
			<div class="container-fluid">
				...
				<p>
				<%=memberId%>의 검색목록 <%=count %>개의 주문 목록
		...
		<form name="search"
			action="/BomBom/manager/productProcess/orderedSearch.jsp"
			method="post">
	...
	<ul class="page">
		...
		<a class="page-link-disable"
			href="orderedSearch.jsp?memberId=<%=memberId%>&pageNum=<%=startPage - pageBlock%>"
			aria-label="Previous">
		...
		%>
		<li class="page-item">
				<a class="page-link" href="orderedList.jsp?memberId=<%=memberId%>&pageNum=<%=i%>">
					<%=i%>
				</a>
			</li>
		<%
