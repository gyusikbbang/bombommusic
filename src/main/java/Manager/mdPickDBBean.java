...
public class mdPickDBBean {
	
    private static mdPickDBBean instance 
                            = new mdPickDBBean();
    
    public static mdPickDBBean getInstance() {
        return instance;
    }
    
    private mdPickDBBean() {}
    ...
    //상품 등록 메소드
    public void insertProduct(mdPickDataBean product) 
    throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"insert into mdPick values (?,?,?,?,?,?,?,?,?,?,?,?,?)");
            pstmt.setInt(1,product.getProCode());
            pstmt.setString(2, product.getProCate());
    ...
    public int searchcount (String proCate,String proName)
		    throws Exception {
		        ...
		        try {
		            ...
		            String sql1 = "SELECT count(*) FROM mdPick WHERE proName LIKE ? ";
		            String sql2 = "select count(*) from mdPick ";
    ...
    public int getProductCount()
    	    throws Exception {
    	        ...
    	        try {
    	            ...
    	            pstmt = conn.prepareStatement("select count(*) from mdPick");
    	            rs = pstmt.executeQuery();
	...
	public List<mdPickDataBean> getProducts(String proCate, int start, int end)
    throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        try {
            ...
            String sql1 = "select * from mdPick order by regdate asc limit ?,?";
            String sql2 = "select * from mdPick ";
	  ...
	  public List<mdPickDataBean> search (String proCate,String proName, int start, int end)
		    throws Exception {
		        Connection conn = null;
		        PreparedStatement pstmt = null;
		        ResultSet rs = null;
		        ...
		        try {
		            ...
		            String sql1 = "SELECT * FROM mdPick WHERE proName LIKE ? order by regdate asc limit ?,?";
		            String sql2 = "select * from mdPick ";
	...
	public mdPickDataBean[] getProducts(String proCate,int count)
    throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        mdPickDataBean productList[]=null;
        int i=0;
        
        try {
            conn = getConnection();
            
            String sql = "select * from mdPick where proCate = ? ";
            ...
            if (rs.next()) {
                productList = new mdPickDataBean[count];
                do{
                     mdPickDataBean product= new mdPickDataBean();
                     product.setProCode(rs.getInt("proCode"));
                     product.setProCate(rs.getString("proCate"));
	...
	public mdPickDataBean getProduct(int proCode)
    throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        mdPickDataBean product=null;
    ...
    public void updateProduct(mdPickDataBean product, int proCode)
    throws Exception {
        Connection conn = null;
        PreparedStatement pstmt = null;
        String sql;
        ...
        try {
            ...
            sql = "update mdPick set proCate=?,ProName=?,Price=?,proLabel=?";
            sql += ",proImg1=?,proImg2=?,proImg3=?,proInfo=?";
            sql += ",proStock=?,concerto=?";
            sql += " where proCode=?";
            ...
            pstmt.setString(4, product.getProLabel());
            
     
            pstmt.setString(5, product.getProImg1());
            pstmt.setString(6, product.getProImg2());
			pstmt.setString(7, product.getProImg3());
			pstmt.setString(8, product.getProInfo());
			
			pstmt.setString(9, product.getProStock());
			pstmt.setBoolean(10, product.isConcerto());

			pstmt.setInt(11, proCode);
    ...
    throws Exception {
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            	"delete from mdPick where proCode=?");
            pstmt.setInt(1, proCode);
    ...
    public int updater(int proCode, String ip) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					"select m.proCode,s.resaveip  From mdPick m LEFT OUTER JOIN resaveip s ON m.proCode = s.proCode = 1 where m.proCode = ?  and s.resaveip = ? ");
			...
			if (rs.next()) {
				do {
				    ...
				    if (dbsaveip.equals(ip)) {
						...
						spl5 = " update mdPick set recommend=recommend-1 where proCode = ? ";
					...
					x = 1; // 인증 성공
						...
						spl2 = " update mdPick set recommend=recommend+1 where proCode = ? ";
 ...
 //추천 판별하기 위한 메소드
	public int recommend(int proCode, String ip) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement(
					"select p.proCode,s.resaveip  From mdPick p LEFT OUTER JOIN resaveip s ON p.proCode = s.proCode = 1 where p.proCode = ?  and s.resaveip = ? ");
	...
	public List<mdPickDataBean> getrankingProducts(String proCate, int start, int end, String select) throws Exception {
	    Connection conn = null;
	    PreparedStatement pstmt = null;
	    ResultSet rs = null;
	    List<mdPickDataBean> productList = null;
	    ...
	    try {
	        ...
	        // 추천 순위
	        String sql1 = "SELECT *, recommend AS total FROM mdPick ORDER BY recommend CAST(recommend AS SIGNED) DESC LIMIT ?,?";
	        String sql2 = "SELECT * FROM mdPick WHERE proCate = ? ORDER BY recommend CAST(recommend AS SIGNED) DESC LIMIT ?,?";

	        // 구매량
	        String sql3 = "SELECT *, buycount AS total FROM mdPick ORDER BY buycount DESC LIMIT ?,?";
	        String sql4 = "SELECT *, buycount AS total FROM mdPick WHERE proCate = ? ORDER BY buycount DESC LIMIT ?,?";

	        // 구매 + 추천
	        String sql5 = "SELECT *, (buycount+recommend) AS total FROM mdPick ORDER BY total CAST((buycount+recommend) AS SIGNED) DESC LIMIT ?,?";
	        String sql6 = "SELECT *, (buycount+recommend) AS total FROM mdPick WHERE proCate = ? ORDER BY total CAST((buycount+recommend) AS SIGNED) DESC LIMIT ?,?";
